...
//   2. Capacitor (native iOS build) serves a COPY assembled into `www/`
//      by `npm run copy-web` (scripts/copy-web.mjs). `webDir` below points
//      at that copy, never at the repo root, so the native shell never sees
//      the mock/dev-only HTML files that litter the root folder.
//
