...
// MOCK (ball-position prototype): third param getWorldOffsetX — optional
// () => metres. geometry-mock.html renders this group inside a wrapper
// shifted +X by state.ballPosition (stance frame), so the DOM label's
// projection anchor (computed in engine frame) must get the same +X shift
// before projecting, or the label drifts off the shifted bracket.
export function createGroundContact(state, labelEl, getWorldOffsetX) {
  ...
  function placeLabel(camera, canvas, towardPositiveX) {
    ...
    _v.copy(anchor);
    if (getWorldOffsetX) _v.x += (getWorldOffsetX() || 0); // MOCK — stance-frame shift
    _v.project(camera);
