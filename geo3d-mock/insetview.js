 ...
 *   getWorldOffsetX — MOCK (ball-position prototype): optional () => metres.
 *               geometry-mock.html renders the engine-driven content inside a
 *               wrapper group shifted +X by state.ballPosition (stance frame:
 *               ball moves, arc stays put). The inset camera pose below is
 *               computed in ENGINE frame, so both its position and its aim
 *               get the same +X shift — which also keeps projectToInset()
 *               correct for UNSHIFTED engine-frame points (shifting camera
 *               and content by the same vector leaves the projection
 *               unchanged).
 */
export function createInsetView({ hideNodes, getWorldOffsetX } = {}) {
  ...
  function updatePose(state) {
    ...
    // MOCK — ball-position stance-frame shift (see getWorldOffsetX doc above):
    // shift camera AND aim by the same +X so the framing follows the shifted
    // content while engine-frame projections stay valid.
    const shiftX = getWorldOffsetX ? (getWorldOffsetX() || 0) : 0;
    camPos.x += shiftX;
    aim.x += shiftX;
