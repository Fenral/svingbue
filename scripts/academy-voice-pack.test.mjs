...
test('development pack truthfully reports caption-ready Academy cues',()=>{
  ...
  assert.equal(report.pass,true);assert.equal(report.cueCount,54);assert.equal(report.assetCount,0);assert.equal(report.captionOnly.length,54);assert.equal(report.rightsStatus,'pending-final-licensed-assets');
});
...
test('release pack fails closed until licensed verified assets exist',()=>{
  ...
  assert.equal(report.pass,false);assert.equal(report.missing.length,54);assert.ok(report.errors.includes('rights-not-approved'));
});
